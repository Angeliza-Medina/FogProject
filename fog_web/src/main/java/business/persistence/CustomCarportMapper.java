...
<<<<<<< HEAD
    ...
    public CustomCarportInquiry getSearchInquiryByLastName(String searchInputLastName) throws UserException {
        CustomCarportInquiry customCarportInquiry = null;
        ...
        try (Connection connection = database.connect()) {
            String sql =
                    " SELECT\n" +
                            ...
                            "    INNER JOIN ccp ON ccp_inquiries.fk_ccp_id = ccp.ccp_id)\n" +
                            "    LEFT JOIN cts ON ccp.fk_cts_id = cts.cts_id)\n" +
                            "WHERE cp_inquiry_statuses.status = ?";

            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setString(3, searchInputLastName);
...
<<<<<<< HEAD
    ...
    public CustomCarportInquiry getSearchInquiryByDate(LocalDate searchInputDate) throws UserException {
        CustomCarportInquiry customCarportInquiry = null;
        ...
        try (Connection connection = database.connect()) {
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ps.setString(2, searchInputDate);
